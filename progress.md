...
| 版本 | 状态 | 说明 |
|------|------|------|
| v0.6.0 | 🟢 Usable | 新增AI翻译分批+本地缓存、可视心跳提示、统一分页与动画反馈，适用于批量翻译/术语管理/ESP写回等全流程场景 |
| v0.5.5 | 🟡 Partially usable | 完成多Session、AI配置、原子/覆盖数据库以及ESP写回（含自动备份）；仍缺完整AI翻译流程，因此标记为可部分使用阶段 |

> 说明：当前桌面端 Tauri 应用以 v0.6.0 为基线，可用于扫描/翻译/术语管理/覆盖排查/写回等场景，后续里程碑将继续迭代 AI 翻译体验。
...
- **AI翻译重复原文自动扩散**（相同原文自动应用相同译文、减少token使用）

### 🚧 当前阶段
**阶段29：AI翻译重复原文自动扩散** - ✅ 已完成

#### 核心功能
- **自动扩散**：当AI提交某条翻译时，自动查找所有原文相同的其他条目并应用相同译文
- **rawText保留**：在预处理阶段保留原始原文用于匹配，避免术语标注影响
- **状态通知**：通过onStatusChange回调通知用户扩散数量
- **无需新工具**：复用现有apply_translations流程，AI不感知扩散逻辑

#### 关键实现

1. **aiPrompts.ts**
   - 扩展`SessionState.csv`类型，增加`rawText`字段

2. **aiTools.ts**
   - `preprocessBatch`返回值增加`rawText`
   - `executeApply`新增可选`expandIndices`参数支持扩散

3. **aiTranslation.ts**
   - 构建`originalTextIndexMap`（原文→索引集合）
   - 创建`expandIndices` resolver函数
   - 调用`executeApply`时传入resolver
   - 状态回调通知扩散数量

#### 遵循原则
- **KISS**：复用现有apply流程，无需新增工具
- **YAGNI**：始终启用扩散，无额外配置
- **DRY**：扩散逻辑集中在executeApply中
